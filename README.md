...
```bash
git clone https://github.com/anantjain341/xcelly.git
cd xcelly
```
...
**6. Run the LangGraph API Server (Terminal 2)**
...
**7. Start the Frontend UI (Terminal 3)**

Open a **third terminal** and start the Next.js React frontend:
```bash
cd web
npm install
npm run dev
...
Open your browser to `http://localhost:3000` to interact with the full Excel Analysis web application!
...
└── web/                         # Next.js Frontend Application
    ...
    ├── components/              # UI elements (ChatPage, FileUpload, etc.)
    └── lib/                     # API interface for frontend SSE streaming
```

### API Overview
The main Brain API (`main.py`) exposes several key endpoints for the frontend interface:
- `POST /api/upload`: Receives multi-part file uploads and returns the file path for analysis context.
- `POST /api/analyze/stream`: The core conversational endpoint. Accepts a JSON payload (`query`, `file_path`, `thread_id`) and returns Server-Sent Events (SSE) streaming updates of the LangGraph state in real-time.
- `GET /plots/{session_id}/{filename}`: Serves generated Matplotlib visualization artifacts dynamically to the frontend.
...
|----------|---------|---------------|
| `OPENAI_API_KEY` | Connects the application to OpenAI GPT-4o for code generation and reasoning (Required) | `sk-...` |
| `ANTHROPIC_API_KEY` | Alternative provider for Claude models | `sk-ant-...` |
| `LANGSMITH_API_KEY` | Enables observability/tracing for LangGraph debugging | `lsv2_...` |
| `LANGSMITH_TRACING` | Toggles LangSmith tracing on or off | `"true"` |
| `LANGSMITH_PROJECT` | Assigns logs to a specific dashboard in LangSmith | `"excel-analysis"` |
| `NEXT_PUBLIC_API_URL` | Used by the frontend (`/web`) to target the main backend API. | `"http://127.0.0.1:8000"` |

*(Note: Infisical and Neo4j keys in `.env.example` are remnants of optional database/secrets hooks not required for the core Excel functionality).*
...
@software{excel_analysis_agent,
  ...
  year = {2024},
  url = {https://github.com/anantjain341/xcelly}
}
```
...
**Anant Jain** - [@anantjain341](https://github.com/anantjain341)

**Project Link**: [https://github.com/anantjain341/xcelly](https://github.com/anantjain341/xcelly)

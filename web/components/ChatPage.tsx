...
export function ChatPage() {
  ...
  const handleSend = useCallback(async () => {
    ...
    abortRef.current = await streamAnalysis(
      backendQuery,
      uploadedFile ? uploadedFile.file_path : null, // Pass actual file path if uploaded
      threadId,
      onEvent,
      onDone,
      onError,
          ...
          <div className="mx-auto max-w-4xl">
            {isEmpty ? (
              ...
              <div className="flex flex-col items-center justify-center h-[calc(100vh-180px)] px-6">
                <div className="flex flex-col items-center gap-8 max-w-lg text-center mt-8">
                  <div className="relative group">
                    <div className="absolute -inset-8 bg-gradient-to-r from-primary/20 via-primary/10 to-primary/20 blur-2xl rounded-full opacity-50 group-hover:opacity-100 transition-opacity duration-700 animate-in fade-in" />
                    <div className="relative flex items-center justify-center h-24 w-24 rounded-3xl bg-gradient-to-br from-background to-muted border justify-center border-primary/20 shadow-xl shadow-primary/5 transition-transform duration-500 group-hover:scale-105">
                      <Sparkles className="h-10 w-10 text-primary drop-shadow-md" />
                    </div>
                  </div>
                  <div className="space-y-3">
                    <h2 className="text-3xl font-bold bg-gradient-to-br from-foreground to-foreground/70 bg-clip-text text-transparent tracking-tight">
                      Report IQ
                    </h2>
                    <p className="text-base text-muted-foreground/90 leading-relaxed font-medium max-w-sm mx-auto">
                      ...
                      <div className="flex flex-wrap gap-2.5 justify-center mt-2">
                        {[
                          ...
                          <button
                            ...
                            className="group relative px-4 py-2 text-[13px] font-medium rounded-full bg-background border border-border/60 hover:border-primary/40 text-muted-foreground hover:text-foreground shadow-sm hover:shadow-md hover:shadow-primary/5 transition-all duration-300"
                          >
                            <span className="relative z-10">{suggestion}</span>
                            <div className="absolute inset-0 rounded-full bg-primary/5 scale-50 opacity-0 group-hover:scale-100 group-hover:opacity-100 transition-all duration-300 -z-0" />
                          </button>
                        ))}
                      </div>
                    </div>
